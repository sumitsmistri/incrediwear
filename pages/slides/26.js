...
function Slide_27() {
  ...
  /* const audio = useAudio("/slides/27/s.mp3", {
    volume: 1,
    playbackRate: 1,
    loop: false,
  }); */
  ...
  return (
    ...
    >
      <Helmet>
        <meta charSet="utf-8" />
        <title>Incrediwear Interactive - 26</title>
      </Helmet>
      {slideData.bg ? (
        <motion.main
          initial={{
          ...
          style={Styles.slide}
          className={`slidePage slidePage page_26`}
        >
          <MenuSite next={27} islight={true} previous={25} />
        </motion.main>
      ) : (
        <Loader />
      )}
