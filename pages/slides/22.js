...
function Slide_23() {
  ...
  /* const audio = useAudio("/slides/23/s.mp3", {
    volume: 1,
    playbackRate: 1,
    loop: false,
  }); */
  ...
  return (
    ...
    >
      <Helmet>
        <meta charSet="utf-8" />
        <title>Incrediwear Interactive - 22</title>
      </Helmet>
      {slideData.bg ? (
        <motion.main style={Styles.slide} className={`slidePage slidePage page_22`}>
          <motion.div
            initial={{ x: 100, opacity: 0 }}
            animate={{ x: 0, opacity: 1 }}
            transition={{ delay: 0.5, duration: 0.5 }}
            className={`container slider-23 bg-white isSlideContentWrapper`}
          >
            <motion.div
            ...
            >
              ...
              >
                <span class="jsx-456089764 text size-2xl color-standard">
                  The heat your body produces anyway is{" "}
                  <em>more than sufficient</em> to <em>power</em>
                  <div style={Styles.blankMargin}></div>
                  <img
                    src={slideData.logo}
                    style={{ maxWidth: "100%" }}
                    alt="Incrediwear Logo"
                    class="jsx-e5423df3a77bdfbd logo"
                  />
                </span>
              </div>
            </motion.div>
          ...
          <MenuSite next={23} islight={false} previous={21} />
        </motion.main>
      ) : (
        <Loader />
      )}
